...
function fun(currentPage) {
    ...
    $.ajax({
        ...
        url: '/hrmsystem/v1/jobInfo/listAllJobInfo',
        dataType: 'json',
        data: postdata,
...
function createShowingTable(data) {
    var tableStr = "<table id='jobTable' class=\"table table-hover\"><thead>";
    tableStr = tableStr
        + "<tr>"
        + "<th class='td-text'>序号</th>"
        + "<th class='td-text'>职位名称</th>"
    ...
    $("#confirmAdd").click(function(){
        ...
        if(param == 0){
            ...
            $.ajax({
                type : 'POST',
                contentType : 'application/json;charset=UTF-8',
                url : '/hrmsystem/v1/jobInfo/addJobInfo',
                dataType : 'json',
                data : postdata,
                success : function(data) {
                    if(data.code == 0){
                        window.location.href = "/hrmsystem/page/jobInfo.jsp";
                    }else{
                        alert(data.showMsg);
                    }
                },
    ...
    $("#confirmEdit").click(function(){
        ...
        if(param == 0){
            ...
            $.ajax({
                type : 'POST',
                contentType : 'application/json;charset=UTF-8',
                url : '/hrmsystem/v1/jobInfo/editJobInfo',
                dataType : 'json',
                data : postdata,
                success : function(data) {
                    if(data.code == 0){
...
$(function () {
   $("#edit").click(function(){
       var jobId = $("tr.active_blue td:eq(1)").text();
       if(jobId == null || jobId == ""){
           $("#editReminder").hide();
           $("#editReminder2").show();
       }else{
...
$(function () {
    $("#confirmDelete").click(function(){
        ...
        $.ajax({
            type : 'POST',
            contentType : 'application/json;charset=UTF-8',
            url : '/hrmsystem/v1/jobInfo/deleteJobInfo',
            dataType : 'json',
            data : postdata,
            success : function(data) {
                if(data.code == 0){
